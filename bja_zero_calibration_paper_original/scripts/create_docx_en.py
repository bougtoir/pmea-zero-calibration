#!/usr/bin/env python3
"""
Create BJA Special Article .docx — English version
...
font.size = Pt(12)
style.paragraph_format.line_spacing = 2.0  # Double-spaced per BJA
...
add_para('Tables and Figures: 1 Table, 4 Figures')
...
# ══════════════════════════════════════════════════════════════════
# SUMMARY (BJA Special Article: unstructured summary)
# ══════════════════════════════════════════════════════════════════
...
add_para(
    ...
    'pressure measurement. Scenario A (offset only, before zeroing) shows reduced CCC (0.817) due '
    'to location shift (u = \u20130.65). Scenario B (after zeroing) shows near-perfect agreement '
    '(CCC = 0.987, Cb = 1.000) because the offset has been removed and no gain error exists. '
    'Scenario C (gain error, v = 0.89) shows that zeroing cannot improve Cb (0.842) because the '
    'scale shift persists. Scenario D (gain + offset) shows the worst case (CCC = 0.573); zeroing '
...
add_figure(
    ...
    '(A) Before zeroing: offset of 12 mmHg produces location shift u = \u20130.65, reducing Cb to 0.828. '
    '(B) After zeroing: offset removed, CCC = 0.987 with Cb = 1.000. '
    '(C) Gain error (v = 0.89): zeroing cannot improve Cb (0.842) because the scale shift persists. '
    '(D) Gain + offset: CCC = 0.573; zeroing would move the device from D to C but not to B. '
